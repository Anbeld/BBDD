...
SELECT rm.registro_civil_matrinomio_id "Identificador del RCM", r.indicativo_serial_rc "Indicativo serial del RCM",
		...
		cont_1.pco_1_nom "Nombres del primer contrayente", cont_1.pco_1_ap "Apellidos del primer contrayente",
		cont_1.pco_1_tip "Tipo de documento del primer contrayente", cont_1.pco_1_doc "Número de documento del primer contrayente",
		cont_2.pco_2_nom "Nombres del segundo contrayente", cont_2.pco_2_ap "Apellidos del segundo contrayente",
		cont_2.pco_2_tip "Tipo de documento del segundo contrayente", cont_2.pco_2_doc "Número de documento del segundo contrayente",
...
-- Para los datos de los contrayentes
LEFT JOIN (SELECT p_cont_1.nombres pco_1_nom, p_cont_1.apellidos pco_1_ap, p_cont_1.numero_documento pco_1_doc,
		   	td_cont_1.tipo_documento pco_1_tip, prm_cont_1.registro_civil_matrinomio_id rcm_id_1,
		   	ROW_NUMBER () OVER (PARTITION BY prm_cont_1.registro_civil_matrinomio_id) cont_1_num
		   	FROM registro_civil_matrimonio rm_cont_1
		   	LEFT JOIN persona_rcm prm_cont_1 ON rm_cont_1.registro_civil_matrinomio_id = prm_cont_1.registro_civil_matrinomio_id 
						AND prm_cont_1.rol_persona_rcm_id = 1
			LEFT JOIN persona p_cont_1 ON prm_cont_1.persona_id = p_cont_1.persona_id
			LEFT JOIN tipo_documento td_cont_1 ON p_cont_1.tipo_documento_id = td_cont_1.tipo_documento_id) cont_1
			ON rm.registro_civil_matrinomio_id = cont_1.rcm_id_1 AND cont_1_num = 1
LEFT JOIN (SELECT p_cont_2.nombres pco_2_nom, p_cont_2.apellidos pco_2_ap, p_cont_2.numero_documento pco_2_doc,
		   	td_cont_2.tipo_documento pco_2_tip, prm_cont_2.registro_civil_matrinomio_id rcm_id_2,
			ROW_NUMBER () OVER (PARTITION BY prm_cont_2.registro_civil_matrinomio_id) cont_2_num
		   	FROM registro_civil_matrimonio rm_cont_2
		   	LEFT JOIN persona_rcm prm_cont_2 ON rm_cont_2.registro_civil_matrinomio_id = prm_cont_2.registro_civil_matrinomio_id 
						AND prm_cont_2.rol_persona_rcm_id = 1
			LEFT JOIN persona p_cont_2 ON prm_cont_2.persona_id = p_cont_2.persona_id
			LEFT JOIN tipo_documento td_cont_2 ON p_cont_2.tipo_documento_id = td_cont_2.tipo_documento_id) cont_2
			ON rm.registro_civil_matrinomio_id = cont_2.rcm_id_2 AND cont_2_num = 2
-- Para los datos del denunciante
...
LEFT JOIN (SELECT rc.indicativo_serial_rc indi_serial_inscrito, prn.persona_id inscrito_id
            ...
            INNER JOIN registro_civil rc ON rn.registro_civil_id = rc.registro_civil_id
            INNER JOIN persona_rcn prn ON rn.registro_civil_nacimiento_id = prn.registro_civil_nacimiento_id
			INNER JOIN rol_persona_rcn rrn ON prn.rol_persona_rcn_id = rrn.rol_persona_rcn_id
